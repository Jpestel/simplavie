...
export async function GET(req: NextRequest) {
  ...
  const rows = await prisma.routineCancellation.findMany({
    where: { userId, date },
    select: { stepId: true },
  })
...
export async function POST(req: NextRequest) {
  ...
  await prisma.routineCancellation.upsert({
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.routineCancellation.deleteMany({ where: { userId, date, stepId } })
  return NextResponse.json({ ok: true })
}
